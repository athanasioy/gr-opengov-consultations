 Select
		...
		coalesce(row_number() OVER (PARTITION by Article.legislation_id,map.MaxArticleToMap order by CAST(Article.number AS INT)) -1 + map.MaxArticleToMap, cast(Article.number as int) as joinNum
 ...
 left join (
	...
	LEFT JOIN ArticleMapping on ArticleMapping.legislation_id = Article.legislation_id
							AND Cast(Article.number as int) >= ArticleMapping.public_consultation_article_no
