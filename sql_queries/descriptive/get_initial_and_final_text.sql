...
FROM Article	
	INNER JOIN Legislation on Legislation.id = Article.legislation_id and Legislation.final_legislation_id is not NULL
	LEFT JOIN ArticleMapping ON ArticleMapping.legislation_id = Article.legislation_id and Cast(Article.number as int) >= ArticleMapping.public_consultation_article_no
